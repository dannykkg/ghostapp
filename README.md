...
> 当前版本：`0.2.4`。交互式扫描显示阶段进度，默认结果使用短摘要；完整结果可导出为模块化工程扫描报告。
...
| HTML 离线报告 | 已支持；亮色工程控制台、模块化展示、可搜索和展开，内嵌完整扫描数据 |
| JSON/AI 接口 | 已支持 |
| 传统 `.app` 深度卸载 | 暂不支持 |
| 恶意软件检测 | 不属于本项目范围 |
...
默认会在当前目录生成带时间戳的 `ghostapp-report-*.html`。报告采用亮色工程扫描控制台布局，把 Provider 分布、扫描管线、风险队列、完整证据、软件清单和诊断结果分模块展示。单文件可搜索软件、展开完整路径和证据，并内嵌完整 Inventory JSON。它不读取凭据或会话正文，不调用网络、数据库、`fetch` 或 `localStorage`；当前用户主目录会显示为 `~`。
